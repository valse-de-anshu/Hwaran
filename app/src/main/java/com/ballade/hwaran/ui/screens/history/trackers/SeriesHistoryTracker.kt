...
object SeriesHistoryTracker {
    fun getSeriesEvents(events: List<HistoryEventEntity>, allMangaMap: Map<Long, MangaEntity>): List<HistoryEventEntity> {
    ...
    fun calculateWatchCount(events: List<HistoryEventEntity>, allMangaMap: Map<Long, MangaEntity>): Int {
        return getSeriesEvents(events, allMangaMap).size
    }

    fun getTimestamps(events: List<HistoryEventEntity>, allMangaMap: Map<Long, MangaEntity>): List<Long> {
        return getSeriesEvents(events, allMangaMap).map { it.timestamp }
    }
}
